...
public class SparseDoubleArray {
	...
	public SparseDoubleArray(int depth, boolean indexing) {
		theList = new NumberedList(0);
		this.depth = depth;
		this.isIndexing = indexing;
		if (indexing) index = new Vector();
	}
	public SparseDoubleArray(int depth) {
		this(depth,true);
	}	
    ...
    */
	...
	/**
	* Tells the double value of this point.
	* @return the value, -1 if p does not exist
	*/
	public double probeValue(int[] p) {
	    int count = 0;
    	list = theList;
    	while (count < depth) {
            int found = Arrays.binarySearch(list.toArray(),new NumberedList(p[count]));
	...
	public void incrementSlow(int[] p, double amt) {
    	int count = 0;
    	list = theList;
    	while (count < depth) {
			int found = -1;
		...
		double i = ((Double)list.get(0)).doubleValue();
		i += amt;
		//list.clear();
		list.set(0,new Double(i));
    }
    //maybe use sorts to help searches...
    public void increment(int[] p, double amt) {
    	int count = 0;
    	list = theList;
    	while (count < depth) {
			int found = Arrays.binarySearch(list.toArray(),new NumberedList(p[count]));
    ...
    public double[] getIndexValues() {
    	int[] point;
    	double[] values = new double[index.size()];
    	int row = 0;
    	Enumeration points = index.elements();
    	while (points.hasMoreElements()) {
    		point = (int[])points.nextElement();
	    	...
	    	values[row] = ((Double)list.get(0)).doubleValue();
	    	row++;
	    }
	    //if (removePermutations) newPoints = removePermutations(newPoints);
	    return values;
	...
	******************/
	...
	public static SparseDoubleArray switchIndices(SparseDoubleArray array, int i, int j) {
	    int[][] index = array.getIntegerIndex();
	    double[] values = array.getIndexValues();
	    SparseDoubleArray newArray = new SparseDoubleArray(array.depth);
	...
	public static SparseDoubleArray sumOverLast(SparseDoubleArray array) {
	    int[][] index = array.getIntegerIndex();
	    double[] values = array.getIndexValues();
	    SparseDoubleArray newArray = new SparseDoubleArray(array.depth-1);
	    for(int k = 0; k < index.length; k++) {
