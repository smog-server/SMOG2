...
public class ShadowMain {
	...
	public static void main(String[] args) {
		...
		ArgParser parser = new ArgParser("java noel.scripts.ContactMap");		
		...
		noel.folding.ContactMap map = null;
		...
		if(multiple.value == 0){
			...
			map = noel.folding.ContactMap.createContactMap(atoms,list,ShadowSettings.SHADOW_RADIUS,ShadowSettings.BONDED_RADIUS,
			...
			if(coarse.value.compareTo("AA")==0 || coarse.value.compareTo("atomic")==0){
				...
				//noel.folding.ContactMap caMap = map.getResidueContacts();
				if(reindex.value == true) {
	...
	// private static void initialize(String groName) {
	// 	printOpener();
	// 	if(ShadowSettings.BIF_PARSING) print2screen("Using protein residue definitions from supplied .bif.");
	// 	print2screen("Reading grofile: "+groName+".");
	// 	grofile = new GroTrajectory(groName);
	// 	atoms = grofile.getAtoms();
	// 	print2screen("Found "+atoms.length+" atoms.");
	// 	if(chains.value!=null){
	// 		print2screen("Reading chains from "+chains.value+".");
	...
	// 	list = topo.getBondedList();
	// }
	/*	private static void initializeGro(String groName) {
		print2screen("Reading grofile: "+groName+".");
		grofile = new GroGro(groName);
		atoms = grofile.getAtoms();
		print2screen("Found "+atoms.length+" atoms.");
		if(chains.value!=null){
		print2screen("Reading chains from "+chains.value+".");
	...
	// private static noel.folding.ContactMap getShadow(double atomsize, double bondedRadius, double cutoff, int rnaDelta, int proteinDelta){
// 		return noel.folding.ContactMap.createContactMap(atoms,list,atomsize,bondedRadius,cutoff,rnaDelta, proteinDelta);
// 	}
// 	private static noel.folding.ContactMap getCutoff(double cutoff,int rnaDelta, int proteinDelta){
// 		return noel.folding.ContactMap.createCutoffContactMap(atoms,list,cutoff,rnaDelta,proteinDelta);
// 	}
    // private static void compareContacts() {
    //  GroGro grofile = new GroGro(gro.value);
    //  atoms = grofile.getAtoms();
    //  noel.folding.ContactMap map1 = new noel.folding.ContactMap(atoms);
    //  noel.folding.ContactMap map2 = new noel.folding.ContactMap(atoms);
    //  map1.addContactFromFile(compare[0]);
    //  map2.addContactFromFile(compare[1]);
    //  Contact[] con = map2.getContacts();
    //  for(int i = 0; i < con.length; i++){
    //      if(map1.connected(con[i].left.getPosition(),con[i].right.getPosition())){
    //          print2screen(con[i].left.getPosition()+" "+con[i].right.getPosition());
    //      }
    //  }
    //  System.exit(0);
    // }
	//returns contacts that are in the 2nd file but not in the 1st file
    // private static void differContacts() {
    //  GroGro grofile = new GroGro(gro.value);
    //  atoms = grofile.getAtoms();
    //  noel.folding.ContactMap map1 = new noel.folding.ContactMap(atoms);
    //  noel.folding.ContactMap map2 = new noel.folding.ContactMap(atoms);
    //  map1.addContactFromFile(differ[0]);
    //  map2.addContactFromFile(differ[1]);
    //  Contact[] con = map2.getContacts();
    //  for(int i = 0; i < con.length; i++){
    //      if(!map1.connected(con[i].left.getPosition(),con[i].right.getPosition())){
    //          print2screen(con[i].left.getPosition()+" "+con[i].right.getPosition());
	...
	static void printOpener() {
		...
		print2screen("More information at: http://smog-server.org/Shadow.html");
