...
/**
 * <b>Edited</b> by Jeff Noel to make Real thread safe 8/1/11.  This was done at the expense of some additional garbage collection.  
 * There were several static temporary variables that are now declared new as necessary in each function.
 * <br><br>
 *
 ...
 *
 *   <li>The library is not thread-safe. <b>(Actually should be now.  See top.)</b> Static <code>Real</code> objects are
...
public final class Real
{
    ...
    private void exp2Internal(long extra) {
        Real expTmp = new Real();
        Real expTmp2 = new Real();
        Real expTmp3 = new Real();
        if ((this.exponent < 0 && this.mantissa != 0))
            return;
        if ((this.exponent < 0 && this.mantissa == 0)) {
            if ((this.sign!=0))
    ...
    private String ftoa(NumberFormat format) {
        byte[] ftoaDigits = new byte[65];
        StringBuffer ftoaBuf = new StringBuffer(40);
        StringBuffer ftoaExp = new StringBuffer(15);
        Real tmp4 = new Real();
        ftoaBuf.setLength(0);
        if ((this.exponent < 0 && this.mantissa != 0)) {
            ftoaBuf.append("nan");
